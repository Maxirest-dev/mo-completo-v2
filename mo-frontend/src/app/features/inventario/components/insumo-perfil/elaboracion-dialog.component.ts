...
import { PasoElaboracion } from '../../models/insumo-perfil.model';
...
export class ElaboracionDialogComponent implements OnInit {
  ...
  startEdit(paso: EditablePaso): void {
    this.originalDescriptions.set(paso.numero, paso.descripcion);
    paso.editing = true;
  ...
  confirmEdit(paso: EditablePaso): void {
    ...
    paso.editing = false;
    this.originalDescriptions.delete(paso.numero);
    this.pasos.update(list => [...list]);
  }

  cancelEdit(paso: EditablePaso, index: number): void {
    const original = this.originalDescriptions.get(paso.numero);
